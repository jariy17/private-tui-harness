import { runExternalCommand } from './ffmpeg.js';
...
const MAX_MAC_GIF_FRAMES = 500;
const VISUAL_MASTER_FILE_NAME = 'visual-master.gif';
...
export async function renderMacVisual(
  ...
  const visualMasterPath = join(workDir, VISUAL_MASTER_FILE_NAME);
  ...
  const scenes = exported.keyframes.map((keyframe, index): DemoMacScene => {
    ...
    const holdMs = quantizeGifDuration(
  ...
  if (frameCount > MAX_MAC_GIF_FRAMES) {
    throw new Error(
      `Mac visual timeline requires ${frameCount} GIF frames, exceeding the ${MAX_MAC_GIF_FRAMES}-frame limit.`
    );
  }
  ...
  const program = buildMacProgram({
    ...
    captionFontSize,
    visualMasterFileName: VISUAL_MASTER_FILE_NAME,
  });
  const manifest: DemoMacManifest = {
    ...
    visualMaster: VISUAL_MASTER_FILE_NAME,
    scenes,
  };
  ...
  const output = await stat(visualMasterPath).catch(() => undefined);
  if (!output?.isFile() || output.size === 0) {
    throw new Error(`Mac completed without creating a visual master at ${visualMasterPath}.`);
  }
...
export function buildMacProgram(options: {
  ...
  lines.push(
    '};',
    `walkthrough.save("${escapeMacString(options.visualMasterFileName)}");`,
    `print "Saved ${escapeMacString(options.visualMasterFileName)}";`,
    ''
  );
  return lines.join('\n');
}
...
function quantizeGifDuration(milliseconds: number): number {
